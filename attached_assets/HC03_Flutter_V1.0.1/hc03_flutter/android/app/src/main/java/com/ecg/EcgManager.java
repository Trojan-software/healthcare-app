...
//                    , ECG_KEY_HRV_FD, ECG_KEY_HRV_TD
        ...
        mNskAlgoSdk.setOnECGAlgoIndexListener((type, value) -> {
            // Log.e("czq"," setOnECGAlgoIndexListener type"+type);
            switch (type) {
...
//                    break;
    ...
    public void dealEcgVal(byte[] bytes) {
        // Log.e("czq","bytes="+bytes);
        for (byte b : bytes) {
            int dataRec = (b & 0xff);
            // Log.e("czq","dataRec="+dataRec);
            if (ecgStageFlag == 0) {
                if (dataRec == 0xaa) {
...
//                                    mEcgListener.onDrawWave(outputPkg);
                        ...
                        // Log.e("czq"," NskAlgoSdk.NskAlgoDataStream ecgDataIndex"+ecgDataIndex+" ecgDataIndex="+ecgDataIndex);
                        //deal data by Algo
