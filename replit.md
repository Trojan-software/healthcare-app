...
- **Multi-Device Bluetooth Integration (November 2025)**: Comprehensive support for all UNKTOP medical peripherals with unified SDK wrapper, auto-pairing workflow, and real-time data synchronization. Supports ECG monitors, blood oxygen sensors (SpO2), blood pressure monitors, blood glucose meters, and temperature probes. Features device-specific dashboard widgets with Chart.js visualizations, DeviceDataContext for centralized state management, and backend API routes for all measurement types with critical alert generation.
