...
**MEDIUM (5 — 4 fixed, 1 accepted/false positive):**
...
- ✅ **WebView Exploits (5.4)**: Added `WebView.setWebContentsDebuggingEnabled(false)` as a static class initializer (fires before any WebView is created), `setSafeBrowsingEnabled(true)`, `setMixedContentMode(MIXED_CONTENT_NEVER_ALLOW)`, and `setMediaPlaybackRequiresUserGesture(true)` to `configureSecureWebView()`. Maximum hardening for a Capacitor/WebView architecture. Needs new APK.
- ✅ **Storing Info in SharedPreferences (6.1)**: Created `TeleHApplication.java` — a custom Application class that overrides `getSharedPreferences()` to transparently wrap ALL preference files (including Capacitor's internal keys) with `EncryptedSharedPreferences` (AES-256-GCM / AES-256-SIV via Android Keystore). Added `androidx.security:security-crypto:1.1.0-alpha06` dependency. Registered `android:name=".TeleHApplication"` in `AndroidManifest.xml`. Needs new APK.
...
- ℹ️ **PhoneGap JavaScript Injection (3.1)**: App uses Capacitor 7.4.3 (latest). The scanner detects the Cordova bridge version string embedded inside Capacitor's `native-bridge.js`. We are already on the latest available version — this is a false positive. HTTPS-only enforced, plain HTTP never used. Recommended: request ADHCC to mark as false positive (same as "Hardcoded Secrets" was overridden).
- ✅ **Keylogger Protection (3.9)**: Addressed with two layers: (1) `webView.setPrivateImeOptions("nm,noExtractUi,noCursorUpdate")` — instructs the IME not to use microphone input or personalised learning. (2) `SecureWebViewClient.onPageFinished()` injects JavaScript that sets `autocomplete="off"`, `autocorrect="off"`, `autocapitalize="off"`, `spellcheck="false"`, `data-lpignore="true"` on ALL input/textarea elements — including dynamically added ones via MutationObserver. Prevents system keyboard and third-party IMEs from caching or transmitting typed data. Needs new APK.
