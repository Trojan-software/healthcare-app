...
-   **Complete HC03 Bluetooth Architecture (Nov 2025)**: 
    - **Web Bluetooth (PWA)**: Full HC03 protocol in `hc03-sdk.ts` with frame unpacking, multi-packet reconstruction, CRC validation (encryHead/encryTail), and all 6 sensor parsers (Battery, Temperature, Glucose, Oxygen, Pressure, ECG). Name-based device filtering (HC02/HC03 prefixes) for maximum compatibility - allows HC02-F1B5D and all HC03 variants to appear in device picker.
    ...
    - **HC02 Device Support (Nov 19, 2025)**: HC02-F1B51D confirmed working with same protocol as HC03. Web Bluetooth filters updated to use name-based matching instead of service UUID requirement, ensuring both HC02 and HC03 devices appear in pairing dialog.
