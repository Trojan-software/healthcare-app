...
function AppContent() {
  ...
  // Fetch latest vital for a single patient
  const fetchLatestVital = async (patientId: string): Promise<VitalSigns | null> => {
    try {
      const r = await authedFetch(`/api/vital-signs/${patientId}`);
      if (!r.ok) return null;
      const rows: VitalSigns[] = await r.json();
      return rows.length > 0 ? rows[0] : null;
    } catch { return null; }
  };

  const formatVitalRow = (v: VitalSigns | null) => {
    if (!v) return { hr: 'N/A', bp: 'N/A', temp: 'N/A', spo2: 'N/A', glucose: 'N/A', ts: 'N/A' };
    const sys = v.bloodPressureSystolic; const dia = v.bloodPressureDiastolic;
    return {
      hr:      v.heartRate  != null ? String(v.heartRate)  : 'N/A',
      bp:      v.bloodPressure || (sys && dia ? `${sys}/${dia}` : 'N/A'),
      temp:    v.temperature != null ? `${parseFloat(String(v.temperature)).toFixed(1)}` : 'N/A',
      spo2:    v.oxygenLevel != null ? String(v.oxygenLevel) : 'N/A',
      glucose: v.bloodGlucose != null ? String(v.bloodGlucose) : 'N/A',
      ts:      v.timestamp  ? new Date(v.timestamp).toLocaleString() : 'N/A',
    };
  };
  ...
  const exportPatientData = async () => {
    const rows = await Promise.all(
      filteredPatients.map(async p => {
        const v = await fetchLatestVital(p.patientId);
        const f = formatVitalRow(v);
        return `${p.patientId},"${p.firstName} ${p.lastName}",${p.email},${p.status},${p.lastActivity},${f.hr},${f.bp},${f.temp},${f.spo2},${f.glucose},${f.ts}`;
      })
    );
    const csv = [
      'Patient ID,Name,Email,Status,Last Activity,Heart Rate (BPM),Blood Pressure (mmHg),Temperature (°C),SpO2 (%),Blood Glucose (mg/dL),Last Reading',
      ...rows
    ].join('\n');
  ...
  const generateWeeklyReport = async () => {
    ...
    const patientLines = await Promise.all(
      filteredPatients.map(async (patient, i) => {
        const v = await fetchLatestVital(patient.patientId);
        const f = formatVitalRow(v);
        const vitalsLine = v
          ? `HR: ${f.hr} BPM | BP: ${f.bp} mmHg | Temp: ${f.temp}°C | SpO2: ${f.spo2}% | Glucose: ${f.glucose} mg/dL`
          : 'No readings recorded';
        return `${i + 1}. ${patient.firstName} ${patient.lastName} (${patient.patientId})
   Status: ${patient.status}  |  Last Activity: ${patient.lastActivity}
   ...
   Reading Date: ${f.ts}`;
    ...
    const reportContent = [
      ...
      `Active Monitoring:    ${stats?.activePatients ?? 0}`,
      `Critical Alerts:      ${stats?.criticalAlerts ?? 0}  (patients with current critical reading)`,
      `Compliance Rate:      ${stats?.complianceRate ?? 0}%`,
      '',
      '──────────────────── VITAL SIGN AVERAGES ───────────────────',
      `Avg Heart Rate:       ${stats?.vitalsAverages?.heartRate ?? 'N/A'} BPM`,
      `Avg Blood Pressure:   ${stats?.vitalsAverages?.bloodPressure ?? 'N/A'}`,
      `Avg Temperature:      ${stats?.vitalsAverages?.temperature ?? 'N/A'}°C`,
      `Avg Oxygen Level:     ${stats?.vitalsAverages?.oxygenLevel ?? 'N/A'}%`,
      '',
      '──────────────────── COMPLIANCE BREAKDOWN ──────────────────',
      `Excellent:            ${stats?.complianceBreakdown?.excellent ?? 0} patients`,
      `Good:                 ${stats?.complianceBreakdown?.good ?? 0} patients`,
      `Needs Improvement:    ${stats?.complianceBreakdown?.needs_improvement ?? 0} patients`,
      '',
      '──────────────────── PATIENT DETAILS ───────────────────────',
      ...patientLines,
      '',
      '──────────────────── ALERT HISTORY ─────────────────────────',
      stats?.alertHistory?.map((a: any) =>
        `${a.type}: ${a.count} incidents (${a.severity} severity)`
      ).join('\n') || 'No alerts recorded',
      '',
      '──────────────────── RECOMMENDATIONS ───────────────────────',
      '• Follow up with any patients showing critical or attention status',
      '• Review medication schedules for patients with abnormal vitals',
      '• Schedule preventive checkups for overdue patients',
      '• Ensure compliance targets are met for all monitored patients',
      '',
      '═══════════════════════════════════════════════════════════',
      '  Report generated by 24/7 Tele H Technology Services',
      '  Contact: support@24x7teleh.com',
      '═══════════════════════════════════════════════════════════',
    ].join('\n');
